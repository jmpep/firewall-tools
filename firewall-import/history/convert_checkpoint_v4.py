...
def flatten_rule(rule, resolver):
    ...
    for key in ['name', 'enabled', 'comments', 'threat-name']:
...
def main():
    ...
    if len(sys.argv) > 1 and '--split' in sys.argv:
        split_mode = True

    if len(args) != 2:
        print("Usage: python convert_checkpoint_v4.py [--split] <json_file> <field1,field2,...>")
        print()
        print("Available fields:")
        print("  rule-number, name, rule-type, policy-name, enabled,")
        ...
        print()
        print("Examples:")
        print('  python convert_checkpoint_v4.py checkpoint_policy.json "rule-number,name,source,destination,action"')
        print('  python convert_checkpoint_v4.py --split checkpoint_policy.json "rule-number,name,source,destination,action"')
        sys.exit(1)
    ...
    output = os.path.splitext(json_file)[0] + '_v4.csv'
